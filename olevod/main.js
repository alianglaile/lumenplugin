...
// 焦点图数据构建函数
function buildHeroBannerData(id, coverURLString, title, descriptionText, detailURLString, backdropURLString) {
  return {
    id: id,
    coverURLString: coverURLString,
    title: title,
...
// HeroBanner 函数：获取焦点图数据
function HeroBanner() {
  // 获取电影和剧集的热门内容作为焦点图
  // API 格式：/v1/pub/vod/list/true/3/0/0/{type}/0/0/update/1/48
  // type: 1=电影, 2=剧集
  const movieURL = "https://api.olelive.com/v1/pub/vod/list/true/3/0/0/1/0/0/update/1/5?_vv=" + signature();
  const tvURL = "https://api.olelive.com/v1/pub/vod/list/true/3/0/0/2/0/0/update/1/5?_vv=" + signature();
  ...
  function tryResolve() {
    ...
    // 交替合并电影和剧集数据
    const mergedItems = [];
    const maxCount = Math.max(movieItems.length, tvItems.length);
    for (let i = 0; i < maxCount; i++) {
      if (i < movieItems.length) {
        mergedItems.push(movieItems[i]);
      }
      if (i < tvItems.length && mergedItems.length < 10) {
        mergedItems.push(tvItems[i]);
      }
    }

    // 限制为10条
    const output = mergedItems.slice(0, 10);
    $next.toMedias(JSON.stringify(output));
  }
  ...
  $http.fetch({
    ...
    try {
      ...
      const content = jsonObj.data.list || [];
      for (let index = 0; index < content.length; index++) {
        const item = content[index];
        const detailURL = buildDetailsURL(item.id.toString());
        const coverURL = buildImageURL(item.pic);
        const title = item.name;
        const descriptionText = item.remarks || "";
        // Use picSlide if available for landscape hero banner, fallback to cover URL (portrait)
        const backdropURL = item.picSlide ? buildImageURL(item.picSlide) : coverURL;
      ...
      print({ scope: "HeroBanner", type: "movie", error: String(e) });
    }
    tryResolve();
  }, function (error) {
    print({ scope: "HeroBanner", type: "movie", error: String(error) });
    tryResolve();
  });
  ...
  $http.fetch({
    ...
    try {
      ...
      print({ scope: "HeroBanner", type: "tv", error: String(e) });
    }
    tryResolve();
  }, function (error) {
    print({ scope: "HeroBanner", type: "tv", error: String(error) });
    tryResolve();
  });
}
